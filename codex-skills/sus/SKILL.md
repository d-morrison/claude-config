---
name: "sus"
description: "Codex wrapper for the ai-config Claude skill `sus`. Alias for `sup` (send-upstream). Send to Upstream: file an issue or open a PR on an upstream repo (a fork's parent, a dependency, or any external project). Use when you've found a bug, want to propose a fix, or need to request a feature in a project you don't own. Handles both 'just report it' (issue) and 'here's the fix' (PR) workflows. Use when asked to 'sus', 'send upstream', 'file upstream issue', 'upstream PR', 'contribute this fix back', or 'report this bug upstream'. Use when Codex is asked to use `sus`, `/sus`, or the corresponding ai-config/Claude skill workflow."
---
...
Source: [skills/sus/SKILL.md](../../skills/sus/SKILL.md)

Before acting, read the source skill completely and follow its workflow, adapting it to Codex.

The source lives at `skills/sus/SKILL.md` in the same ai-config checkout as this wrapper. If this wrapper was loaded through `${CODEX_HOME:-$HOME/.codex}/skills/sus`, resolve the symlink target for this wrapper directory first, then read `../../skills/sus/SKILL.md` relative to that real directory. Do not resolve that relative path from inside `${CODEX_HOME:-$HOME/.codex}/skills`, because it points back at the wrapper tree.
