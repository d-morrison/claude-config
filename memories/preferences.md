...
- ALWAYS run a local session in an isolated `git worktree`, never directly in the shared
  working copy — this is the default for EVERY local session, not just substantial multi-file
  work or when the user flags the wd as "in use" / "do this in a separate repo". The ai-config
