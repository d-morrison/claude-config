...
If the suffix isn't PDT/PST, fall back to plain `date` when the machine's system zone is already Pacific, or use PowerShell (output format will differ from the bash command — it's a raw `DateTime` with no timezone-abbreviation field, so format it yourself if you need the `PDT`/`PST` suffix or a compact form): `[System.TimeZoneInfo]::ConvertTimeBySystemTimeZoneId([DateTime]::UtcNow, 'Pacific Standard Time')`.
...
When writing new `.qmd` prose, add line breaks at major phrase and sentence boundaries (one `.qmd` prose, break at sentence boundaries; one clause per line works well.
...
```{r}
#| label: stage-at-dx-fig #| code-fold: true
