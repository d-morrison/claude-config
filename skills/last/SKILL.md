...
- A task added with `/last` sits behind all `/also` tasks and *stays* there —
  ...
  Think "always run this at the very end."

## How to handle it

1. **Place the task at the very end** of the queue, behind any existing `/also`
   tasks.
2. **Keep it pinned there.**
   When a new `/also` arrives later, insert that `/also` *in front of* the `/last` task, not after it.
...
4. **Run it dead last**, once everything else (including later `/also` work) is
   ...
   Report its outcome in the final wrap-up.

## Edge cases

- **Nothing else queued.**
  It is still last — which, with an otherwise empty queue, just means "do it after the current task."
- **A `/first` or `/next` arrives later.**
  Those jump ahead (head / after-current as usual); the `/last` task stays at the very end regardless.
