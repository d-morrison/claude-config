...
5. **If it's a skill** — create it in `~/.claude/skills/` (symlink to the
   cloned repo; discover the repo path with `git -C ~/.claude/skills/record-learnings rev-parse --show-toplevel`)

## Sharing with other agents

The `~/.claude/skills/` directory is a symlink to wherever you cloned
`ai-config` (discover the path with `git -C ~/.claude/skills/record-learnings rev-parse --show-toplevel` —
point `-C` at a skill subdir, since `bootstrap.sh` may symlink skills per-child
so the parent itself isn't in the repo).
